...
- **Provider lockfile caching in iac-driver** - The `.state/tofu/{env}-{node}/data/` directories cache tofu provider lockfiles. When provider version constraints change (e.g., Dependabot PRs), stale lockfiles cause validation failures. Clear state directories or add preflight check for version mismatches.
