...
**Always include a doc verification issue.** Every sprint should have a "Verify docs for {Theme}" issue in homestak-dev using the [doc-verification.md](../templates/doc-verification.md) template. This verifies that documentation (CLAUDE.md, design docs, memory files) accurately reflects the sprint's final state. Create it during planning so it's not forgotten at the end. See [issues.md](../standards/issues.md#issue-placement) for issue placement conventions.
